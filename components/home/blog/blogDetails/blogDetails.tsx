...
const BlogDetails = ({ slug }: Props) => {
    ...
    const { data: recommendedBlogsResp, isLoading: fetchingRecommended } = useGetAllRecommendedArticles()
                            ...
                            <li key={index}>
                                <div className={styles.recommended_blog}>
                                    <div className={styles.blog_recommended_img}>
                                        <CustomImage width={400} height={450} src={blog?.bannerImage ?? ""} alt={blog.title} />
                                    </div>
                                    <div className={styles.tags_time_container}>
                                        <ul className={styles.tags_container}>
                                            <li className={styles.tag}>{blog?.category?.name}</li>
                                        </ul>
                                        <span className={styles.time_container}><Image src="/svgs/clock.svg" alt='clock' height={20} width={20} /> <p className={styles.item}> {blog?.readMinutes || 0} mins Read</p></span>
                                    </div>
                                    <div className={styles.recommended_blog_content}>
                                        <h2 className={styles.blog_title}>{blog.title}</h2>
                                        <p className={styles.blog_preview}>
                                            <p dangerouslySetInnerHTML={{ __html: shortenTitle(blog.content.text, 600) ?? "" }} />
                                        </p>
                                    </div>
                                </div>
                            </li>
