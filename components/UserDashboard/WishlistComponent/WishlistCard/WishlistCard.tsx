...
const WishlistCard = ({
	...
	const forSellPrice =
		item?.listingType === "sell" ? item?.offer?.forSell?.pricing : "";
	const forRentPrice =
		item?.listingType === "rent"
			? item?.offer?.forRent?.rates.length
				? item?.offer?.forRent?.rates[0].price
				: 0
			: "";
		...
		>
			<div>
				{item.type === "courses" ? (
					<>
						...
						<div className={styles.container__details__detail_container}>
							...
							<p className={`${styles.value} ${styles.listing_type}`}>
								{item.listingType}
							</p>
						</div>
						<div className={styles.container__details__detail_container}>
							<p className={styles.key}>Category</p>
							<p className={`${styles.value} ${styles.category}`}>
								{item.category}
							</p>
						</div>
					</>
					...
					<>
						<div className={styles.container__details__detail_container}>
							<p className={styles.key}>
								{item.type === "sell" ? "Seller" : "Lender"}
							</p>
							<p className={styles.value}>
								<div className={styles.listing_user}>
									<Image
										src={user?.avatar || "/svgs/avatar-user.svg"}
						...
						<div className={styles.container__details__detail_container}>
							...
							<p className={`${styles.value} ${styles.availability}`}>
								{item.status}
							</p>
						</div>
					</>
				...
				<div className={styles.container__details__detail_container}>
					...
					>
						<Link href={`/user/listings/${item._id}`}>See details</Link>
					</Button>
					{deletingItem ? (
						<LoadingSpinner size="small" />
					) : (
						<p
							className={`${styles.value} ${styles.action_icons}`}
